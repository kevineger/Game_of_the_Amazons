...
//                    sendToServer(GameMessage.ACTION_MOVE, roomID);
            ...
            case GameMessage.MSG_GAME:
              ...
              //  move queenMove = translate.translateIn(queen_move1);
                translate.translateIn(queen_move1);

                for (Queen q: bl.getEnemies()) {
                    if (q.colPos == translate.inOldCol && q.rowPos == translate.inOldRow) {
                        q.move(translate.inNewRow, translate.inNewCol);
                        bl.updateAfterMove();
                    }
                }

                System.out.println("Move from position: "+translate.inOldCol+", "+translate.inOldRow+" to "+translate.inNewCol+", "+translate.inNewRow);

                Arrow a = translate.translateArrowIn(arrow_move1);
                bl.addArrow(a);
                bl.updateAfterMove();

                move queenMove = new move(translate.inNewCol, translate.inNewRow, translate.inOldCol, translate.inOldRow);

                frame.update(queenMove, a, true);
                bl.updateAfterMove();
                System.out.println(bl.toString());
                bl = randomMove(bl);
                bl.updateAfterMove();
